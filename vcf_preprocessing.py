...
if __name__ == "__main__":
    ...
    all_GTs_array.drop(all_GTs_array[all_GTs_array.QUAL < 1000].index, inplace=True)
    all_GTs_array = all_GTs_array.astype(str)
    all_GTs_array = all_GTs_array[~all_GTs_array[all_GTs_array == "./."].any(axis=1)]
    # pickle.dump(all_GTs_array, open('vcf_original_only_PASS_SNPs_QUAL1000_no_NULL', 'wb'))
    # df[~(df < 0.5).all(1)]
    ...
    pickle.dump(all_GTs_array, open('vcf_original_only_PASS_SNPs_QUAL1000_no_NULL.p', 'wb'))
